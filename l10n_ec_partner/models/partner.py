...
class ResPartner(models.Model):
    '''agrega campos para cedula/ruc/pasaporte, escoge dependiendo del digito 3 del numero si es juridica, natural u otro
       '''
    ...
    def _compute_tipo_persona(self):
        '''179.. ruc para empresas, 170... ruc/cedula pnatural'''
        if not self.identifier:
            self.tipo_persona = '0'
        elif int(self.identifier[2]) <= 6:
            print("es cedula %s" %self.identifier)
            self.tipo_persona = '6'
        elif int(self.identifier[2]) in [6, 9]:
            print("es ruc %s" % self.identifier)
            self.tipo_persona = '9'
        else:
            self.tipo_persona = '0'
