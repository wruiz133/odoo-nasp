    ...
    def create_picking(self):
        ...
        for inv in self:
            if any([ptype in ['product', 'consu'] for ptype in inv.invoice_line_ids.mapped('product_id.type')]):  # noqa
                ...
                moves = moves.filtered(lambda x: x.state not in ('done', 'cancel'))._action_confirm()  # noqa
                # moves.force_assign() no existe en stock.move
                moves._compute_reserved_availability()
                picking.do_unreserve
                #picking.do_transfer() no existe en stoc.picking
        return True
