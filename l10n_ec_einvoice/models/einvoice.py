...
class AccountInvoice(models.Model):
    ...
    def _info_factura(self, invoice):
        """cuando llega aca ya debe exitir infofactura que esta en moduo edocument.py
        todos los datos estan en relacion al cliente y las sumas de los valores del detalle
        """
        def fix_date(date):
        ...
        infoFactura = {
            'fechaEmision': fix_date(invoice.date_invoice), #fecha convertida a dd/mm/yyyy
        ...
        for tax in invoice.tax_line_ids:
            ...
            if tax.group_id.code in ['vat', 'vat0', 'ice']:
                totalImpuesto = {
                    ...
                    'baseImponible': '{:.2f}'.format(tax.invoice_id.amount_vat_cero),#'{:.2f}'.format(tax.base),
        ...
        if self.type == 'out_refund': # ssi nc en factura de ventas add la estructura necesaria a infofactura
    ...
    def _detalles(self, invoice):
        """ hace un remplazo en el iterable special (encode remplaza ñes % *) usadas en campo descripcion
    ...
    def render_document(self, invoice, access_key, emission_code):
        ''''__file__ como parametro da el path de templates buscando desde donde se ejecuta el programa
        ...
        einvoice_tmpl = env.get_template(self.TEMPLATES[self.type]) # obtiene el out_invoice.xml
        data = {}
    ...
    def action_generate_einvoice(self):
        ...
        for obj in self:
            ...
            access_key, emission_code = self._get_codes(name='account.invoice') # access_key validada y tipo emision
            einvoice = self.render_document(obj, access_key, emission_code) # xml segun sri (xsd)
            #print("///////einvoice/////////={0}".format(einvoice))
            #print("///////einvoice/////////=", einvoice)
            inv_xml = DocumentXML(einvoice, obj.type) # instancia clase de sri.py
            ...
            signed_document = xades.sign(einvoice, file_pk12, password) # ejecuta java -jar ...jar ....
            signed_document1 = str(signed_document, 'utf-8') # bytes a str wr
            print('lo que sale luego de firmarse  y se  espera autorizacion sri /////////////    ', signed_document1)
            ...
            if ambiente=='1' and modo:
                ok, errores = inv_xml.send_receipt(signed_document1, obj.company_id.recepcion_pruebas_offline)
                if not ok:
                    raise UserError(errores)
            if ambiente=='1' and not modo:               
                ok, errores = inv_xml.send_receipt(signed_document1, obj.company_id.recepcion_pruebas_online)
                if not ok:
                    raise UserError(errores)
            if ambiente=='1' and modo:
            ...
            #
            # attach = self.add_attachment(auth_einvoice, auth,file)
            # message = """
