...
class HrContract(models.Model):
    ...
    def _compute_days(self):
        for obj in self:
            ds = datetime.strptime(str(obj.date_start), DEFAULT_SERVER_DATE_FORMAT).date()  # noqa
            de = datetime.now().date()
            if obj.date_end:
                de = datetime.strptime(str(obj.date_end), DEFAULT_SERVER_DATE_FORMAT).date()  # noqa
            days = de - ds
